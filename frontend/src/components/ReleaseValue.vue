<template><span class="value"><span :class="{ technical }">{{ shown }}</span> <button v-if="shortened" type="button" :aria-label="`${revealed ? 'Hide' : 'Reveal'} ${label}`" @click="revealed = !revealed">{{ revealed ? 'Hide' : 'Reveal' }}</button><button v-if="technical && available" type="button" :aria-label="`Copy ${label}`" @click="copy">{{ copied ? 'Copied' : 'Copy' }}</button><span v-if="failed" role="status">Copy unavailable; reveal and select the value.</span></span></template>
<script setup>
import { computed, ref } from 'vue'
import { displayValue } from '../composables/releaseDetails.js'
const props = defineProps({ value: [String, Number], label: { type: String, required: true }, technical: { type: Boolean, default: true } })
...
const shortened = computed(() => props.technical && /^[0-9a-f]{32,}$/i.test(text.value))
