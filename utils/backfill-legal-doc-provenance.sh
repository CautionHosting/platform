#!/bin/bash
# SPDX-FileCopyrightText: 2025 Caution SEZC
# SPDX-License-Identifier: AGPL-3.0-only OR LicenseRef-Commercial

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "$SCRIPT_DIR/../.env" ]]; then
    source "$SCRIPT_DIR/../.env"
fi

DB_HOST="${DB_HOST:-localhost}"
DB_PORT="${DB_PORT:-5432}"
DB_NAME="${DB_NAME:-caution}"
DB_USER="${DB_USER:-postgres}"
DB_PASSWORD="${DB_PASSWORD:-postgres}"

export PGPASSWORD="$DB_PASSWORD"

SHA_REGEX='^[0-9a-fA-F]{7,40}$'
DATELESS_VERSION_REGEX='^.+$'

usage() {
    cat <<EOF
Backfill source provenance onto an existing legal_documents row.

Usage:
  ./utils/backfill-legal-doc-provenance.sh \\
    --website-repo /path/to/caution/website \\
    --document-type terms_of_service|privacy_notice \\
    --version <existing-version> \\
    --source-path terms.md|privacy.md \\
    --commit <sha> [options]

Options:
  --force        Overwrite existing provenance fields
  --help         Show this message

Requires local commands: git, psql, sed, awk, and a SHA-256 tool.
EOF
}

die() {
    echo "Error: $*" >&2
    exit 1
}

require_command() {
    command -v "$1" >/dev/null 2>&1 || die "Required command not found: $1"
}

psql_cmd() {
    psql -v ON_ERROR_STOP=1 -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -A "$@"
}

sql_quote() {
    printf "%s" "$1" | sed "s/'/''/g"
}

hash_stdin() {
    if command -v sha256sum >/dev/null 2>&1; then
        sha256sum | awk '{print $1}'
    elif command -v shasum >/dev/null 2>&1; then
        shasum -a 256 | awk '{print $1}'
    elif command -v openssl >/dev/null 2>&1; then
        openssl dgst -sha256 -r | awk '{print $1}'
    else
        die "No SHA-256 tool found (need sha256sum, shasum, or openssl)"
    fi
}

validate_regex() {
    local value="$1"
    local regex="$2"
    local message="$3"
    if ! [[ "$value" =~ $regex ]]; then
        die "$message"
    fi
}

WEBSITE_REPO=""
DOCUMENT_TYPE=""
VERSION=""
SOURCE_PATH=""
COMMIT_SHA=""
FORCE="false"

while [[ $# -gt 0 ]]; do
    case "$1" in
        --website-repo)
            WEBSITE_REPO="${2:-}"
            shift 2
            ;;
        --document-type)
            DOCUMENT_TYPE="${2:-}"
            shift 2
            ;;
        --version)
            VERSION="${2:-}"
            shift 2
            ;;
        --source-path)
            SOURCE_PATH="${2:-}"
            shift 2
            ;;
        --commit)
            COMMIT_SHA="${2:-}"
            shift 2
            ;;
        --force)
            FORCE="true"
            shift
            ;;
        --help|-h)
            usage
            exit 0
            ;;
        *)
            die "Unknown argument: $1"
            ;;
    esac
done

require_command git
require_command psql
require_command sed
require_command awk

[[ -n "$WEBSITE_REPO" ]] || die "--website-repo is required"
[[ -n "$DOCUMENT_TYPE" ]] || die "--document-type is required"
[[ -n "$VERSION" ]] || die "--version is required"
[[ -n "$SOURCE_PATH" ]] || die "--source-path is required"
[[ -n "$COMMIT_SHA" ]] || die "--commit is required"

[[ -d "$WEBSITE_REPO/.git" ]] || die "Not a git repo: $WEBSITE_REPO"
validate_regex "$COMMIT_SHA" "$SHA_REGEX" "--commit must look like a git SHA"
validate_regex "$VERSION" "$DATELESS_VERSION_REGEX" "--version must be non-empty"

case "$DOCUMENT_TYPE" in
    terms_of_service|privacy_notice) ;;
    *)
        die "--document-type must be terms_of_service or privacy_notice"
        ;;
esac

if ! git -C "$WEBSITE_REPO" rev-parse --verify "${COMMIT_SHA}^{commit}" >/dev/null 2>&1; then
    die "Commit not found in website repo: $COMMIT_SHA"
fi

if ! git -C "$WEBSITE_REPO" cat-file -e "${COMMIT_SHA}:${SOURCE_PATH}" 2>/dev/null; then
    die "File not found at commit: ${COMMIT_SHA}:${SOURCE_PATH}"
fi

TARGET_ID="$(psql_cmd -c "
    SELECT id
    FROM legal_documents
    WHERE document_type = '$(sql_quote "$DOCUMENT_TYPE")'
      AND version = '$(sql_quote "$VERSION")'
    LIMIT 1;
")"

[[ -n "$TARGET_ID" ]] || die "No legal_documents row found for ${DOCUMENT_TYPE} version=${VERSION}"

EXISTING_FIELDS="$(psql_cmd -F $'\t' -c "
    SELECT
        COALESCE(source_commit_sha, ''),
        COALESCE(source_path, ''),
        COALESCE(content_sha256, '')
    FROM legal_documents
    WHERE id = '$(sql_quote "$TARGET_ID")';
")"

IFS=$'\t' read -r EXISTING_COMMIT EXISTING_PATH EXISTING_HASH <<<"$EXISTING_FIELDS"

if [[ "$FORCE" != "true" && ( -n "$EXISTING_COMMIT" || -n "$EXISTING_PATH" || -n "$EXISTING_HASH" ) ]]; then
    die "Target row already has provenance set. Re-run with --force to overwrite."
fi

CONTENT_SHA256="$(git -C "$WEBSITE_REPO" show "${COMMIT_SHA}:${SOURCE_PATH}" | hash_stdin)"

CONFLICT_ID="$(psql_cmd -c "
    SELECT id
    FROM legal_documents
    WHERE document_type = '$(sql_quote "$DOCUMENT_TYPE")'
      AND content_sha256 = '$(sql_quote "$CONTENT_SHA256")'
      AND id <> '$(sql_quote "$TARGET_ID")'
    LIMIT 1;
")"

if [[ -n "$CONFLICT_ID" ]]; then
    die "Another ${DOCUMENT_TYPE} row already uses content_sha256=${CONTENT_SHA256} (id=${CONFLICT_ID})"
fi

psql_cmd -c "
    UPDATE legal_documents
    SET
        source_path = '$(sql_quote "$SOURCE_PATH")',
        source_commit_sha = '$(sql_quote "$COMMIT_SHA")',
        content_sha256 = '$(sql_quote "$CONTENT_SHA256")'
    WHERE id = '$(sql_quote "$TARGET_ID")';
" >/dev/null

echo "Backfilled legal document provenance:"
echo "  id:              $TARGET_ID"
echo "  document_type:   $DOCUMENT_TYPE"
echo "  version:         $VERSION"
echo "  source_path:     $SOURCE_PATH"
echo "  source_commit:   $COMMIT_SHA"
echo "  content_sha256:  $CONTENT_SHA256"
