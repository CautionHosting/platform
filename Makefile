...
.PHONY: build-all build-enclave network postgres migrate run-api run-api-test run-gateway run-gateway-test run-email-test up up-test down down-clean down-test logs clean clean-enclave build-cli build-cli-host install-cli install-cli-stagex install-cli-host release-cli sign-cli verify-cli reproduce-cli test test-unit test-cli-install test-e2e test-e2e-ssh-units test-e2e-pgp-units test-e2e-pgp-audit test-e2e-platform-ports test-e2e-legal test-e2e-webauthn test-e2e-webauthn-roundtrip test-e2e-webauthn-browser test-e2e-org-user-quorum test-e2e-byoc test-e2e-billing-gates test-e2e-paddle-subscriptions test-paddle-sandbox build-gateway-e2e postgres-test migrate-test prepare-byoc-provisioner build-frontend-dist build-hcl-patcher clean-e2e
...
test-e2e:
	...
	if [ $$status -eq 0 ]; then \
		CAUTION_BIN="$(PWD)/$(CLI_OUT_DIR)/$(CLI_BINARY)" bash tests/e2e/test_ssh_units.sh || status=$$?; \
	fi; \
	if [ $$status -eq 0 ]; then \
		bash tests/e2e/test_pgp_units.sh || status=$$?; \
	fi; \
	if [ $$status -eq 0 ]; then \
		bash tests/e2e/test_pgp_audit_constraints.sh || status=$$?; \
	fi; \
	$(MAKE) down-test; \
	exit $$status
...
test-e2e-pgp-units:
	@$(MAKE) up-test
	@echo "Running PGP units e2e tests..."
	@bash tests/e2e/test_pgp_units.sh; \
	status=$$?; \
	$(MAKE) down-test; \
	exit $$status
...
test-e2e-pgp-audit:
	@$(MAKE) up-test
	@echo "Running PGP audit/constraint e2e tests..."
	@bash tests/e2e/test_pgp_audit_constraints.sh; \
	status=$$?; \
	$(MAKE) down-test; \
	exit $$status
