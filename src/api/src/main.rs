...
fn enforce_generic_resource_limit(self_managed_capacity_reserved: bool) -> bool {
    !self_managed_capacity_reserved
}

#[cfg(test)]
mod self_managed_plan_capacity_tests {
    use super::{byoc_capacity_permits_new_deployment, enforce_generic_resource_limit};
    ...
    #[test]
    fn reserved_self_managed_capacity_bypasses_the_generic_limit() {
        assert!(!enforce_generic_resource_limit(true));
        assert!(enforce_generic_resource_limit(false));
    }
}
...
async fn deploy_logic(
    ...
    if is_managed_onprem {
        ...
        let reserved = sqlx::query(
            "UPDATE compute_resources
             SET destroyed_at = NULL, state = $1, deploy_attempt_id = $2
             WHERE id = $3 AND organization_id = $4
               AND state != $1 AND state <> 'terminating'",
        )
        .bind(types::ResourceState::Pending)
        .bind(deploy_attempt_id)
        .bind(resource_id)
    ...
    if enforce_generic_resource_limit(self_managed_capacity_reserved) {
